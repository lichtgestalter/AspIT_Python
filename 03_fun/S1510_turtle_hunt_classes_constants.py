...
class PlayerName1(turtle.Turtle):
    ...
    def rotate_prey(self, positions):  # turtle will be turned right <degree> degrees. Use negative values for left turns.
        ...
        #closet_hunter = min(positions[1:])
        distances = [distance(positions[0], position) for position in positions[1:]]
        directions = [direction(positions[0], position) for position in positions[1:]]
        closest_hunter = distances.index(min(distances))
        print(directions[closest_hunter])

        degree = directions[closest_hunter] - self.orientation + 180  # When the turtle rotates the same amount each turn,  it will just run in a circle. Make this function smarter!
    ...
    def rotate_hunter(self, positions):  # turtle will be turned right <degree> degrees. Use negative values for left turns.
        ...
        degree = direction(self.position(), positions[0]) - self.orientation  # When the turtle rotates the same amount each turn,  it will just run in a circle. Make this function smarter!
        self.orientation += degree
        self.orientation %= 360
