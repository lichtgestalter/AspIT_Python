...
def main():
    ...
    while not dead(morris) and morris["turn"] < turns:
        ...
        morris["turn"] += 1
        for attribute in morris:
            print(f"{attribute}: {morris[attribute]}")
        ...
        questions = [
            inquirer.List('activity',
                          ...
                          choices=['sleep', 'mine', 'eat', 'buy whisky', 'drink'],
                          ),
        ]
        answer = inquirer.prompt(questions)["activity"]
        os.system("cls")

        for activity in activitys:
            if activity == answer:
                activitys[activity](morris)
    ...
    if dead(morris):
        ...
        print(figlet_format(f"Morris survived {turns} turns and earned {morris['gold']} gold", font="bulbhead", width=150))
